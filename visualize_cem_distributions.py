...
for ax_idx, (title, samples_raw, samples_clamp, sigma_norm) in enumerate([
    ...
    ax.set_xlabel('X (m) — vorwärts/rückwärts', fontsize=12)
    ax.set_ylabel('Y (m) — links/rechts', fontsize=12)
...
plt.tight_layout()
output_path_1 = os.path.join(os.path.dirname(__file__), "cem_distribution_top_view.png")
plt.savefig(output_path_1, dpi=150, bbox_inches='tight')
print(f"Gespeichert: {output_path_1}")
...
plt.tight_layout()
output_path_2 = os.path.join(os.path.dirname(__file__), "cem_distribution_side_view.png")
...
for col, (dim_name, dim_idx, ws_lo, ws_hi) in enumerate([
    ...
    ax.set_xlabel(f'{dim_name} (m)', fontsize=12)
...
plt.tight_layout()
output_path_3 = os.path.join(os.path.dirname(__file__), "cem_distribution_1d_marginals.png")
plt.savefig(output_path_3, dpi=150, bbox_inches='tight')
print(f"Gespeichert: {output_path_3}")
