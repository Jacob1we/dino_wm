...
def create_fig1_xy_iterations():
    fig = plt.figure(figsize=(TEXTWIDTH_IN, TEXTWIDTH_IN * 0.4))
    ...
    for col, step_data in enumerate(CEM_ITERATIONS):
        ...
        ax_main.legend(loc='lower right', framealpha=0.8)
        ...
        ax_top.set_title(step_data['label'], fontweight='bold', color=color, pad=8)
        ax_top.set_yticks([])
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "ma_cem_xy_iterations")
    save_ma_figure(fig, path)
    plt.close()
    print(f"✓ {path}")
...
def create_fig2_z_iterations():
    fig = plt.figure(figsize=(TEXTWIDTH_IN, TEXTWIDTH_IN * 0.37))
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "ma_cem_z_iterations")
    save_ma_figure(fig, path)
    plt.close()
    print(f"✓ {path}")
...
def create_fig3_xy_sigma_scales():
    fig, axes = plt.subplots(1, 3, figsize=(TEXTWIDTH_IN, TEXTWIDTH_IN * 0.37))
    fig.suptitle(
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "ma_cem_xy_sigma_scales")
    save_ma_figure(fig, path)
    plt.close()
    print(f"\u2713 {path}")
...
def create_fig4_z_sigma_scales():
    ...
    for col, (scale, color, title) in enumerate(SCALES):
        ...
        ax_main.grid(True, alpha=0.2)
        ax_main.legend(loc='upper left', framealpha=0.8)
        ax_main.set_title(title, color=color, fontweight='bold')
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "ma_cem_z_sigma_scales")
    save_ma_figure(fig, path)
    plt.close()
    print(f"✓ {path}")
...
def create_fig5_xy_timesteps():
    ...
    for col, ts in enumerate(MPC_TIMESTEPS):
        ...
        ax_top.set_ylabel('p(x)')
        ax_top.tick_params(labelbottom=False, labelsize=6)
        ax_top.set_title(ts['label'], fontweight='bold', color=color, pad=8)
        ax_top.set_yticks([])
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "ma_cem_xy_timesteps")
    save_ma_figure(fig, path)
    plt.close()
    print(f"✓ {path}")
...
def create_fig6_z_timesteps():
    ...
    plt.tight_layout()
    path = os.path.join(OUTPUT_DIR, "ma_cem_z_timesteps")
    save_ma_figure(fig, path)
    plt.close()
    print(f"✓ {path}")
