...
$T_\text{hist} = 4$ Frames aus **derselben Episode wie das gewählte DINO-Bild** werden als
Kontext bereitgestellt. Dabei wird kein zufälliger `TrajSlicerDataset`-Eintrag verwendet,
sondern ein explizites Zeitfenster aus der vollständigen Episode konstruiert, sodass der
angefragte Episode-Frame konsistent im WM-Kontext liegt.

Das Fenster folgt exakt der Trainingskonvention des `TrajSlicerDataset`:

$$\text{obs indices} = [s,\; s + f,\; s + 2f,\; \ldots,\; s + (T_\text{hist}-1)f]$$

$$\text{act chunk}_i = \mathbf{a}[s + i f : s + (i+1)f]$$

mit $f = \text{frameskip}$ und einem Startindex $s$, der so gewählt wird, dass der gewünschte
Episode-Frame nach Möglichkeit **exakt** Teil des Kontexts ist.

Jeder Kontext-Frame wird durch den DINOv2 Encoder (mit `encoder_transform`) zu 196 Patch-Tokens
...
#### Schritt 3: Self-Attention des ausgewählten Kontext-Frames extrahieren

Aus der Attention-Matrix wird der Block des **tatsächlich visualisierten** Kontext-Frames isoliert.
Ist der gewünschte Episode-Frame z.B. der dritte Frame im Kontext, wird genau dessen Token-Block
verwendet und nicht pauschal der letzte Kontext-Frame.

$$\mathbf{A}_\text{sel} = \mathbf{A}_\text{pred}[t_k\!:\!t_k\!+\!196,\; t_k\!:\!t_k\!+\!196] \in \mathbb{R}^{196 \times 196}$$

mit

$$t_k = k \times 196, \quad k \in \{0, 1, 2, 3\}$$

wobei $k$ die Position des gewählten Episode-Frames innerhalb des Kontexts ist.
...
$$\mathbf{s}_i = \sum_{j=1}^{196} A_\text{sel}[j, i] \quad \Rightarrow \quad \mathbf{S} \in \mathbb{R}^{14 \times 14}$$
...
- **Original Frame** + **Self-Attention Summe** des ausgewählten Episode-Frames (Mean über alle Heads)
...
Die ViT- und Rekonstruktions-Plots sind jetzt an dieselbe Episode gekoppelt wie die DINO-Plots.
Die Dateinamen referenzieren den tatsächlich visualisierten Episode-Frame. Dadurch entsteht keine
stille Verschiebung mehr durch die zufällige Permutation des `TrajSlicerDataset`.
...
- `visualize_proprio_trajectory()` wurde fuer die 3-Panel-Abbildung (3D, x-y, z)
  auf feste Masterarbeit-Breite (`TEXTWIDTH_IN`) umgestellt.
- Der horizontale Abstand zwischen den drei Panels wurde erhoeht (`wspace=0.5`),
  damit Achsenbeschriftungen nicht mehr kollidieren.
- `run_all_visualizations.py` erzeugt WM-Sanity-Episodenplots nun ebenfalls in
  fixer Breite (`TEXTWIDTH_IN`) statt variabler Breite `3 * n_show`.
- Fuer WM-Sanity-Exporte wird `save_ma_figure(..., fixed_canvas=True)` genutzt,
  damit `.svg` und `_unlabeled.svg` bei gleicher Figure-Canvas exakt dieselbe
  Breite behalten.
- Bei `--skip_existing` wird ein Sanity-Ergebnis nur dann als "vorhanden"
  gewertet, wenn sowohl beschriftete `.svg` als auch `_unlabeled.svg` Dateien
  existieren.
- `wm_sanity_check.py` wurde analog auf fixe Breite umgestellt, damit Standalone-
  und Batch-Ausfuehrung identisches Exportverhalten haben.
