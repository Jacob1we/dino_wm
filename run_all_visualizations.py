...
Für jedes Modell werden die Ergebnisse abgelegt:
  feature_visualizations/<model_name>/
    feature_visualizations/<model_name>/wm_sanity_check/
...
def run_feature_visualization(model_name, model_path, output_dir, device,
    ...
    from visualize_features import (
        ...
        visualize_proprio_embedding, visualize_proprio_trajectory,
    )
    ...
    if hasattr(model, 'proprio_encoder') and model.proprio_encoder is not None and episode_indices:
        print("  [+] Proprio Visualisierungen...")
        for ep_idx in episode_indices[:2]:  # max 2 Episoden für Proprio
            if ep_idx >= len(dset):
                continue
            obs_raw, actions, _, _ = dset[ep_idx]
            obs_proprio = obs_raw.get("proprio", None)
            ...
            visualize_proprio_embedding(
                ...
                os.path.join(output_dir, f"proprio_embedding_{proprio_tag}.png"),
                proprio_mean=pm, proprio_std=ps,
            )
            visualize_proprio_trajectory(
                model, dset, ep_idx, device,
                os.path.join(output_dir, f"proprio_trajectory_{proprio_tag}.png"),
...
def run_sanity_check_for_model(model_name, model_path, output_dir, device,
    ...
    for ep_num, dset_idx in enumerate(selected):
        ...
        save_ma_figure(
            fig,
            os.path.join(output_dir, f"episode_{dset_idx:04d}"),
            fixed_canvas=True,
        )
        plt.close(fig)
    ...
    if pred_mses_by_step:
        ...
        save_ma_figure(
            fig,
            os.path.join(output_dir, "mse_over_horizon"),
            fixed_canvas=True,
        )
        plt.close(fig)
...
def main():
    ...
    for idx, (model_name, model_path) in enumerate(all_models):
        ...
        sanity_dir = os.path.join(feat_dir, "wm_sanity_check")

        # Skip-Prüfung
        if args.skip_existing:
            feat_exists = os.path.isdir(feat_dir) and len(os.listdir(feat_dir)) > 0
            sanity_files = os.listdir(sanity_dir) if os.path.isdir(sanity_dir) else []
            sanity_has_labeled = any(f.endswith(".svg") and not f.endswith("_unlabeled.svg") for f in sanity_files)
            sanity_has_unlabeled = any(f.endswith("_unlabeled.svg") for f in sanity_files)
            sanity_exists = sanity_has_labeled and sanity_has_unlabeled
        ...
        if not args.skip_sanity:
            ...
            try:
                sanity_files = os.listdir(sanity_dir) if os.path.isdir(sanity_dir) else []
                sanity_has_labeled = any(
                    f.endswith(".svg") and not f.endswith("_unlabeled.svg")
                    for f in sanity_files
                )
                sanity_has_unlabeled = any(
                    f.endswith("_unlabeled.svg")
                    for f in sanity_files
                )
                sanity_exists = sanity_has_labeled and sanity_has_unlabeled

                if args.skip_existing and sanity_exists:
